# Lab Control Framework - Communication Protocol Specification
...
4. [ZMQ Communication Patterns](#4-zmq-communication-patterns)
5. [TCP Communication Patterns](#5-tcp-communication-patterns)
6. [JSON Message Formats](#6-json-message-formats)
7. [Component-Specific Protocols](#7-component-specific-protocols)
...
## 4. ZMQ Communication Patterns

### 4.1 Command Flow (Manager → Workers)
...
### 4.3 Client Request Flow (Flask ↔ Manager)
...
## 5. TCP Communication Patterns

### 5.1 LabVIEW SMILE Control (Port 5559)
...
## 6. JSON Message Formats

### 6.1 ZMQ Command Messages (Port 5555)
...
### 6.2 ZMQ Data Messages (Port 5556)
...
### 6.3 Client Request/Response (Port 5557)
...
```json
{
  ...
  "mode": "MANUAL",
  "worker_alive": true,
  "current_exp": "EXP_240128_A1B2C3D4",
  "params": {...},
  "turbo": {
    "status": "idle",
    "iteration": 0,
...
|----------|-----------|---------|
| SET_DC | Manager → ARTIQ | Set electrode voltages |
| SET_COOLING | Manager → ARTIQ | Set Raman parameters |
| SET_RF | Manager → ARTIQ/LabVIEW | Set RF voltage |
| SET_PIEZO | Manager → ARTIQ/LabVIEW | Set piezo voltage |
| SET_DDS | Manager → ARTIQ | Set DDS profile |
| RUN_SWEEP | Manager → ARTIQ | Execute frequency sweep |
| HEARTBEAT | ARTIQ → Manager | Health check |
| DATA | ARTIQ → Manager | Generic data |
| SWEEP_COMPLETE | ARTIQ → Manager | Sweep finished |
| STATUS | ARTIQ → Manager | Status update |
| ERROR | ARTIQ → Manager | Error notification |
| SAFETY_TRIGGER | Any → Manager | Safety event |
